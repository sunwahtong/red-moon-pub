(() => {
  const boot = () => {
    ...
    function fitScale() {
      ...
      const pad = window.innerWidth <= 800 ? 18 : 32;
    ...
    function zoomAt(next, clientX, clientY) {
      ...
      // Keep the point under the mouse/finger fixed while zooming.
    ...
    window.addEventListener('resize', () => {
      const before = baseScale;
      fitScale();
      // Preserve the current map zoom and pan when the viewport changes.
      if (before !== baseScale) render();
    });

    viewport.addEventListener('wheel', e => {
      e.preventDefault();
      const direction = e.deltaY < 0 ? STEP : -STEP;
      zoomAt(zoom + direction, e.clientX, e.clientY);
    }, { passive: false });
    ...
    viewport.addEventListener('mouseleave', e => { if (dragging) stop(e); });

    document.querySelectorAll('[data-map-zoom-in]').forEach(b =>
      b.addEventListener('click', () => centerZoom(STEP))
    );
    document.querySelectorAll('[data-map-zoom-out]').forEach(b =>
      b.addEventListener('click', () => centerZoom(-STEP))
    );
    document.querySelectorAll('[data-map-reset]').forEach(b =>
