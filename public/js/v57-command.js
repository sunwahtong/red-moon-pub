...
(() => {
  ...
  async function renderDocuments(){
    ...
    try{const d=await api('/api/documents');const docs=d.documents||[];const invoices=docs.filter(x=>x.type==='invoice'),receipts=docs.filter(x=>x.type==='receipt');
      ...
      host.querySelectorAll('[data-delete-receipt]').forEach(b=>b.onclick=async()=>{if(!await confirmBox('Nyugta törlése',`Biztosan törlöd a(z) ${b.dataset.deleteReceipt} nyugtát? Az eredeti eladás nem törlődik.`))return;try{await api('/api/documents/'+encodeURIComponent(b.dataset.deleteReceipt),{method:'DELETE'});toast('Nyugta törölve','A nyugta eltávolításra került.');renderDocuments()}catch(e){toast('Törlés sikertelen',e.message,true)}});
